...
onMounted(() => {
  const ctx = gsap.context(() => {
    gsap.from('.route-metrics .metric-card', {
      y: 20, opacity: 0, duration: 0.6, stagger: 0.08, ease: 'power3.out',
    })
    gsap.from('.page-hero', {
      y: 16, opacity: 0, duration: 0.5, ease: 'power3.out', delay: 0.2,
    })
    gsap.from('.route-main > *:not(.page-hero)', {
      y: 24, opacity: 0, duration: 0.6, stagger: 0.1, ease: 'power3.out', delay: 0.35,
    })
    gsap.from('.route-aside > *', {
      y: 20, opacity: 0, duration: 0.5, stagger: 0.1, ease: 'power3.out', delay: 0.5,
    })
  })
  return () => ctx.revert()
})
      ...
      <div class="agent-search-main route-main min-w-0">
        ...
        <section class="panel micro-panel">
          ...
          <div class="micro-grid">
            <article
              ...
              :key="item"
              :class="`tone-${['green','blue','violet','cyan','teal','violet'][index]}`"
            >
              <span>{{ item }}</span>
        ...
        <section class="panel detail-table">
          ...
          <table>
            <thead>
              <tr>
                <th>方案</th><th>航行距离</th><th>预计时间</th><th>燃油消耗</th><th>节油率</th><th>风险等级</th><th>综合评分</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(row, index) in plans" :key="row[0]" :class="{ 'optimal-row': index === 0 }">
                <td><span v-if="index === 0" class="optimal-tag">★ {{ row[0] }}</span><span v-else>{{ row[0] }}</span></td>
                <td>{{ row[1] }} <small class="unit">km</small></td>
                <td>{{ row[2] }} <small class="unit">h</small></td>
                <td>{{ row[3] }} <small class="unit">吨</small></td>
                <td><span v-if="index === 0" class="highlight-value">{{ row[4] }}</span><span v-else>{{ row[4] }}</span></td>
                <td><span :class="['risk-badge', 'risk-' + row[5]]">{{ row[5] }}</span></td>
                <td><span v-if="index === 0" class="star-rating best">{{ row[6] }}</span><span v-else>{{ row[6] }}</span></td>
              </tr>
            </tbody>
          </table>
        </section>
          ...
          <div class="chat-body">
            <div class="chat-messages">
              ...
              <div v-if="isLoading" class="chat-message assistant">
                ...
                <div class="message-content loading"><Loader2 :size="16" class="spinner" /> 思考中...</div>
              </div>
            </div>
            <div class="chat-input-area">
              <input type="text" v-model="chatInput" placeholder="输入您的问题..." @keyup.enter="sendMessage" :disabled="isLoading" />
              <button @click="sendMessage" :disabled="isLoading || !chatInput.trim()"><Send :size="18" /></button>
            </div>
          </div>
        </div>
      </div>
